...
export function WorkWeekView({ 
  ...
  // Get start of work week (Monday)
  const startOfWorkWeek = new Date(currentDate)
  const dayOfWeek = currentDate.getDay()
  const daysToMonday = dayOfWeek === 0 ? -6 : 1 - dayOfWeek // Sunday = 0, so go back 6 days
  startOfWorkWeek.setDate(currentDate.getDate() + daysToMonday)
  
  // Generate work week days (Monday to Friday)
  const workWeekDays = []
  for (let i = 0; i < 5; i++) {
    const day = new Date(startOfWorkWeek)
    day.setDate(startOfWorkWeek.getDate() + i)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Work week content - detailed view */}
      <div className="grid grid-cols-5 gap-1 overflow-x-auto">
        {workWeekDays.map((day, index) => {
          const isToday = day.toDateString() === today.toDateString()
          ...
          return (
            <div
              key={index}
              className={cn(
                "min-h-[500px] p-3 border rounded-lg cursor-pointer hover:bg-muted/30 transition-colors",
                isToday && "bg-blue-50 border-blue-200",

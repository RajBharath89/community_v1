...
import { VolunteerActionButtons } from "./action-buttons"
import { VolunteerStatsCards } from "./stats-cards"
import { VolunteerSearchFilters } from "./search-filters"
import { VolunteerTableContent } from "./volunteer-table-content"
import { VolunteerTablePagination } from "./volunteer-table-pagination"
import { VolunteerDrawer } from "./volunteer-drawer"
import { VolunteerAdvancedFilterDrawer } from "./advanced-filter-drawer"
import { useVolunteerStore } from "@/stores/volunteer-store"

function VolunteerManagement() {
  const { viewMode } = useVolunteerStore()
    ...
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
          <div className="relative z-10 ml-20 sm:ml-28 flex-1 min-w-0">
            ...
            <p className="text-xs sm:text-sm lg:text-base text-gray-600 mt-1">
              Track and manage volunteer activities
            </p>
          </div>
      ...
      <div className="bg-white rounded-lg shadow">
        <VolunteerTableContent 
          ...
          viewMode={viewMode}
        />
        <VolunteerTablePagination />
      </div>
      
      {/* Drawers */}
      <VolunteerDrawer />
      <VolunteerAdvancedFilterDrawer />
    </div>
  )
}
